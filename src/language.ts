const en = {
  MESSAGE_CONNECTED: 'Successfully connected!',
  MESSAGE_NOT_FOUND: 'Not found. Try again',
  MESSAGE_TOO_LARGE:
    'You can not download musics with more than 15 minutes duration',
  MESSAGE_WAIT_QUEUE:
    'Download proccess got taken by another user. Await then finish to continue *BOT BY NISHI*',
  MESSAGE_DOWNLOAD_STARTED:
    '*DOWNLOAD STARTED:* _This action can take some minutes_ *BOT BY NISHI*',
  MESSAGE_DOWNLOAD_ERROR: 'Error! Try again',
  MESSAGE_NO_DOWNLOADED_MUSICS: 'No stored musics',
  MESSAGE_ERROR: 'Error to complete operation.',
  COMMAND_PLAY: '!play',
};

export default { en };
